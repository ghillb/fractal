...
describe("repository capability surface", () => {
  test("exposes a stable derived schema version invariant", () => {
    ...
    expect(surface.schemaVersionBoundaryStability.value).toBe(
      `repository-capability-surface-boundary-stability/v${REPOSITORY_CAPABILITY_SURFACE_VERSION}`
    );
    ...
    expect(() => {
      (surface.schemaVersionBoundaryStability as { value: string }).value = "mutated";
    }).toThrow();
    ...
    expect(surface.schemaVersionSurfaceSchemaFingerprint.value).toBe(
      `repository-capability-surface-surface-schema-fingerprint/v${REPOSITORY_CAPABILITY_SURFACE_VERSION}`
    );
    expect(Object.isFrozen(surface.schemaVersionSurfaceSchemaFingerprint)).toBe(true);
    expect(() => {
      (surface.schemaVersionSurfaceSchemaFingerprint as { value: string }).value = "mutated";
    }).toThrow();
    ...
    expect(surface.schemaVersionSurfaceImmutability.value).toBe(
      `repository-capability-surface-surface-immutability/v${REPOSITORY_CAPABILITY_SURFACE_VERSION}`
    );
    expect(Object.isFrozen(surface.schemaVersionSurfaceImmutability)).toBe(true);
    expect(() => {
      (surface.schemaVersionSurfaceImmutability as { value: string }).value = "mutated";
    }).toThrow();
    ...
    expect(surface.schemaVersionSchemaStabilityChecksum.value).toBe(
      `repository-capability-surface-schema-stability-checksum/v${REPOSITORY_CAPABILITY_SURFACE_VERSION}`
    );
