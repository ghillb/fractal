...
describe("repository capability surface", () => {
  test("exposes a stable derived schema version invariant", () => {
    ...
    expect(surface.schemaVersionSurfaceBoundarySignature).toBe(schemaVersionSurfaceBoundarySignature);
    ...
    expect(() => {
      (surface.schemaVersionSurfaceImmutabilityWitnessReadOnly as { value: string }).value = "mutated";
    }).toThrow();
    expect(() => {
    ...
    expect(() => {
      (surface.schemaVersionSurfaceDerivedImmutabilityShape as { schemaVersion: number }).schemaVersion = 5;
    }).toThrow();
    expect(() => {
