...
describe("capability snapshot", () => {
  test("exports a versioned immutable aggregate of read-only entrypoints and validation hooks", () => {
    ...
    expect(() => {
      (capabilitySnapshot.surface as { publicShapeVersion: number }).publicShapeVersion = 2;
    }).toThrow();
    expect(() => {
    ...
    expect(() => {
      (capabilitySnapshot.surface as { exportVisibilityVersion: number }).exportVisibilityVersion = 2;
    }).toThrow();
    expect(() => {
